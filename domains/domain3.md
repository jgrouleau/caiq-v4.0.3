---
layout: default
title: Domain 3 - Business Continuity, Management, and Operational Resilience
domain: "Business Continuity, Management, and Operational Resilience"
summary_path: "/domains/domain3-summary"
previous_domain_path: "/domains/domain2-summary"
---
...
function showQuestion(qidx, total) {
  ...
  if (current) {
    ...
    if (qidx <= 1) {
      ...
      // We don't necessarily need to disable it, as its href is set correctly.
      // We could add visual indication if needed, but for now, just setting the href is enough for navigation.
    } else {
      prev.href = "?q={{ forloop.index | minus: 1 }}"; // This might need adjustment if we loop over questions differently
    }
