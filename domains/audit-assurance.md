...
slug: "audit-assurance" # Keep slug for lookup
...
{% assign questions = site.data.questions | where: "domain", current_domain.name %}
...
{% for q in questions %}
  <div class="question-block" data-qidx="{{ forloop.index }}" data-domain="{{ q.domain }}" style="display:none;">
    ...
    <div style="display: flex; justify-content: space-between; max-width: 700px; margin: 0 auto 2em auto;">
      ...
      {% if forloop.last %}
        <a href="{{ current_domain.summary_page | relative_url }}" class="btn btn-primary nav-btn nav-next">Review &rarr;</a>
      {% else %}
...
function showQuestion(qidx, total) {
  ...
  if (current) {
    ...
    if (qidx <= 1) {
      ...
      // The href is already set in Liquid using previous_domain.summary_page.
